...
fun BukaShiftScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.event.collect { event ->
            when (event) {
                BukaShiftEvent.ShiftBerhasilDibuka -> onShiftBerhasil()
                BukaShiftEvent.KembaliKeLogin -> onGantiKasir()
                is BukaShiftEvent.Pesan -> snackbarHostState.showSnackbar(event.teks)
            }
        }
    }
