...
class EscPosPrinterDriver(
            ...
            if (!logoUri.isNullOrBlank()) {
                loadAndConvertLogo(logoUri)?.let { logoBytes ->
                    try {
                        ...
                        Log.w(TAG, "Failed to send logo: ${e.message}")
                    }
                }
            }
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    if (fileSize != null && fileSize > MAX_LOGO_FILE_SIZE_BYTES) {
                        Log.w(TAG, "Logo file too large: $fileSize bytes (max: $MAX_LOGO_FILE_SIZE_BYTES)")
                    ...
                    if (bounds.outWidth <= 0 || bounds.outHeight <= 0) {
                        Log.w(TAG, "Logo dimensions invalid: ${bounds.outWidth}x${bounds.outHeight}")
                        return@withContext null
                    }

                    if (bounds.outWidth > MAX_SOURCE_DIMENSION || bounds.outHeight > MAX_SOURCE_DIMENSION) {
                        Log.w(TAG, "Logo too large: ${bounds.outWidth}x${bounds.outHeight} (max: $MAX_SOURCE_DIMENSION)")
                        return@withContext null
                    }
                    ...
                    Log.w(TAG, "No permission to read logo: ${e.message}")
                    null
                } catch (e: Exception) {
                    Log.w(TAG, "Failed to load logo: ${e.message}")
                    null
                }
            }
        }
    ...
    private fun getFileSize(uri: Uri): Long? {
        return try {
            ...
            Log.w(TAG, "Cannot get file size for $uri: ${e.message}")
            null
        }
    }
