        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                if (selectedTab == 1 && inputJumlah.isNotBlank()) {
                    val tanda = if (selisihOpname > 0) "+${formatQuantity(selisihOpname)}" else formatQuantity(selisihOpname)
                    Surface(
                        shape = MaterialTheme.shapes.medium,
                        color = if (selisihOpname >= 0) MaterialTheme.colorScheme.secondaryContainer else MaterialTheme.colorScheme.errorContainer,
                        modifier = Modifier.fillMaxWidth(),
                    ) {
                        Row(
                            ...
                            Text("Selisih Opname:", style = MaterialTheme.typography.bodySmall)
                            Text(
                                "$tanda unit",
                                fontWeight = FontWeight.Bold,
                                style = MaterialTheme.typography.bodySmall,
                                color = if (selisihOpname >= 0) MaterialTheme.colorScheme.onSecondaryContainer else MaterialTheme.colorScheme.onErrorContainer,
                            )
                        }
                    }
                }
