...
class ProductManagementService(
    ...
    suspend fun createProduct(request: CreateProductRequest, now: Long): Result<Long> = suspendRunCatching {
    ...
    suspend fun updateProduct(request: UpdateProductRequest, now: Long): Result<Unit> = suspendRunCatching {
        val namaClean = request.nama.trim()
        val skuClean = request.sku.trim()
        val barcodeClean = request.barcode?.trim()?.takeIf { it.isNotBlank() }
        ...
        write.run {
            val existing = produkDao.getById(request.id)
    ...
    suspend fun deactivateProduct(productId: Long, now: Long): Result<Unit> = suspendRunCatching {
        write.run {
    ...
    suspend fun activateProduct(productId: Long, now: Long): Result<Unit> = suspendRunCatching {
        write.run {
            produkDao.getById(productId) ?: throw PosDataException.NotFound("Produk tidak ditemukan")
    ...
    suspend fun restock(request: RestockRequest, now: Long): Result<Unit> = suspendRunCatching {
    ...
    suspend fun adjustStockOpname(request: StockOpnameRequest, now: Long): Result<Unit> = suspendRunCatching {
    ...
    suspend fun recordDamage(request: RecordDamageRequest, now: Long): Result<Unit> = suspendRunCatching {
    ...
    suspend fun disposeDamage(request: DisposeDamageRequest, now: Long): Result<Unit> = suspendRunCatching {
