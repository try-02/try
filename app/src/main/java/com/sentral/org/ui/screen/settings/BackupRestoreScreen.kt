...
fun BackupRestoreScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            // Tombol Aksi Backup
            Button(
                onClick = { viewModel.bukaDialogBackup() },
            ...
            // Tombol Aksi Restore
            OutlinedButton(
                onClick = { viewModel.mulaiProsesRestorePilihFile() },
                enabled = !uiState.sedangMemproses,
                shape = MaterialTheme.shapes.medium,
                modifier = Modifier.fillMaxWidth().height(52.dp),
            ) {
    ...
    if (uiState.dialogBackupTerbuka) {
        DialogPasswordBackup(
            ...
            onCatatanChange = viewModel::setCatatan,
            onKonfirmasi = viewModel::konfirmasiSiapExport,
            onDismiss = viewModel::tutupDialogBackup,
        )
    }
    ...
    if (uiState.dialogPasswordRestoreTerbuka) {
        DialogPasswordRestore(
            ...
            pesanError = uiState.pesanError,
            onPasswordChange = viewModel::setPassword,
            onKonfirmasi = viewModel::verifikasiDanBukaPreviewRestore,
            onDismiss = viewModel::tutupDialogPasswordRestore,
        )
    }
    ...
    if (uiState.dialogPreviewRestoreTerbuka) {
        uiState.previewMetadata?.let { meta ->
            DialogPreviewRestore(
                metadata = meta,
                onKonfirmasi = viewModel::komitRestoreDanMuatUlang,
                onBatal = viewModel::batalPreviewRestore,
            )
        }
    }
}
