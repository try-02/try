...
class ReturServiceIntegrationTest {
    ...
    private suspend fun seedProduk(
        sku: String,
        harga: Long,
        stok: Long,
    ): Long {
    ...
    private suspend fun checkoutLangsung(
        ...
        val now = currentTimeMillis()
        val cartId =
            db.keranjangDao().insert(
                KeranjangEntity(
                    nama = "Cart",
                    status = StatusKeranjang.AKTIF,
                    kasirId = sesi.kasirId,
                    namaKasir = "Kasir Retur",
                    dibuatPada = now,
                    diperbaruiPada = now,
                    ditahanPada = null,
        ...
        return checkoutService
            .checkout(
                CheckoutRequest(
                    cartId = cartId,
                    cashierId = sesi.kasirId,
                    shiftId = sesi.shiftId,
                    payments = listOf(payment),
                    transactionNumber = nomor,
                    now = now,
                ),
            ).getOrThrow()
    }

    private data class TxSeed(
        ...
        val txId =
            db.transaksiDao().insert(
                TransaksiEntity(
                    ...
                    nilaiDiskon = diskonItem,
                    status = StatusTransaksi.SELESAI,
                    dibatalkanPada = null,
                    alasanPembatalan = null,
                    adalahTukarGaransi = false,
                ),
            )
        val itemId =
            db
    ...
    private suspend fun itemIdPertama(transaksiId: Long): Long =
        db
            .itemTransaksiDao()
            .getByTransaction(transaksiId)
            .first()
            .id

    @Test
    fun returNormalMengembalikanStokMengurangiKasDanTercatatLengkap() =
        runTest {
                ...
                returService
                    .process(
                        ReturnRequest(
                            ...
                            refundMethod = MetodePembayaran.CASH,
    ...
    @Test
    fun returRusakMasukKeSaldoRusakTanpaMenyentuhStokNormal() =
        runTest {
            val sesi = seedKasirShift()
    ...
    @Test
    fun returDisposeTidakMengubahPersediaanMaupunKasFisik() =
        runTest {
            val sesi = seedKasirShift()
    ...
    @Test
    fun returBertahapTidakPernahMelebihiNilaiNetoBaris() =
        runTest {
            val sesi = seedKasirShift()
            val produkId = seedProduk("RET-BUDGET", harga = 101, stok = 10_000)
            val seed =
                seedTransaksiPresisi(
                    sesi,
                    produkId,
                    "TRX-OVR",
                    jumlah = 3_000,
            ...
            suspend fun sesiRetur(): ReturnResult =
                returService
                    .process(
                        ReturnRequest(
                            transactionId = seed.transaksiId,
                            cashierId = sesi.kasirId,
                            shiftId = sesi.shiftId,
    ...
    @Test
    fun returMelebihiSisaQuantityDitolakTanpaJejak() =
        runTest {
            val sesi = seedKasirShift()
    ...
    @Test
    fun returPadaTransaksiVoidDitolak() =
        runTest {
            val sesi = seedKasirShift()
