...
interface PrinterDao {
    ...
    @Query("SELECT * FROM printer ORDER BY prioritas, id")
    fun observeAll(): Flow<List<PrinterEntity>>

    @Query("SELECT * FROM printer WHERE id = :id LIMIT 1")
    suspend fun getById(id: Long): PrinterEntity?

    @Query("SELECT * FROM printer WHERE is_default = 1 ORDER BY prioritas, id LIMIT 1")
    suspend fun getDefault(): PrinterEntity?

    @Query("UPDATE printer SET gagal_status_berturut=:failures, dinonaktifkan_otomatis=:disabled WHERE id=:id")
