...
class OfflineCartRepository(
    ...
    override fun observeItemsLive(cartId: Long): Flow<List<BarisKeranjangLive>> = items.observeLiveByCart(cartId)
    ...
    override suspend fun hold(
        cartId: Long,
        now: Long,
    ) = carts.hold(cartId, now)

    override suspend fun resume(
        cartId: Long,
        now: Long,
    ) = carts.resume(cartId, now)

    override suspend fun cancel(
        cartId: Long,
        now: Long,
    ) = carts.cancel(cartId, now)
}
