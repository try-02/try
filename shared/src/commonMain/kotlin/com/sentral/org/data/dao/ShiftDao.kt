...
interface ShiftDao {
    ...
    @Query("SELECT * FROM shift WHERE kasir_id = :kasirId AND status = 'TERBUKA' ORDER BY id DESC LIMIT 1")
    suspend fun getOpenForKasir(kasirId: Long): ShiftEntity?

    @Query(
        "UPDATE shift SET status = 'DITUTUP', kas_diharapkan = :expected, kas_aktual = :actual, selisih_kas = :difference, ditutup_pada = :waktu, catatan = :catatan WHERE id = :id AND status = 'TERBUKA'",
    )
    suspend fun close(
        id: Long,
        expected: Long,
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM shift WHERE kasir_id = :kasirId AND status = 'TERBUKA')")
    suspend fun hasOpenForKasir(kasirId: Long): Boolean
