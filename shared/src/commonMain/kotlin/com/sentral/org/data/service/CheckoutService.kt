...
class CheckoutService(
     ...
     */
    suspend fun checkout(request: CheckoutRequest): Result<CheckoutResult> =
        suspendRunCatching {
            write.run {
                // ---------- VALIDASI ----------
                val cart =
                    carts.getById(request.cartId)
                ...
                val shift =
                    shifts.getById(request.shiftId)
...
*/
    ...
    private fun validatePayments(
        ...
        val cashChange =
            inputs
                .filter { it.method == MetodePembayaran.CASH }
