...
interface ProdukDao {
    ...
    @Query("UPDATE produk SET nama = :nama, sku = :sku, barcode = :barcode, harga = :harga, harga_modal = :hargaModal, kategori = :kategori, diperbarui_pada = :waktu WHERE id = :id") suspend fun updateMaster(id: Long, nama: String, sku: String, barcode: String?, harga: Long, hargaModal: Long, kategori: String, waktu: Long): Int
    ...
    suspend fun existsBySku(sku: String, excludeId: Long? = null): Boolean

    @Query("SELECT EXISTS(SELECT 1 FROM produk WHERE barcode = :barcode AND (:excludeId IS NULL OR id != :excludeId))")
    suspend fun existsByBarcode(barcode: String, excludeId: Long? = null): Boolean
