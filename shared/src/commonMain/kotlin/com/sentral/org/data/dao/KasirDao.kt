...
interface KasirDao {
    ...
    @Query("SELECT * FROM kasir WHERE id = :id LIMIT 1")
    suspend fun getById(id: Long): KasirEntity?

    @Query("SELECT * FROM kasir WHERE aktif = 1 ORDER BY nama COLLATE NOCASE")
    fun observeAktif(): Flow<List<KasirEntity>>

    @Query("UPDATE kasir SET aktif = :aktif WHERE id = :id")
