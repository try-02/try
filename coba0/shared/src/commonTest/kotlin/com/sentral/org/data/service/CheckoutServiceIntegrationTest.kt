...
class CheckoutServiceIntegrationTest {
    ...
    @Test
    fun checkoutSuksesMenulisSemuaJejakDenganBenar() =
        runTest {
            val f = seedStandar()
    ...
    @Test
    fun kegagalanDiTengahTransaksiMengembalikanSemuaTulisan() =
        runTest {
            val f = seedStandar()
            ...
            assertTrue(
                db.pergerakanPersediaanDao().getByProduk(f.produkIds[0]).none {
            ...
            assertEquals(
                0,
                db.pergerakanKasDao().getByShift(f.shiftId).count { it.jenis == JenisPergerakanKas.PENJUALAN },
            )
        }

    @Test
    fun checkoutGandaSerentakHanyaSatuYangBerhasil() =
        runTest {
            val f = seedStandar()
            ...
            assertEquals(
                1,
                db.pergerakanPersediaanDao().getByProduk(f.produkIds[0]).count {
                    it.jenis == JenisPergerakanPersediaan.PENJUALAN
                },
            )
            ...
            assertEquals(
                ...
                db
                    .pergerakanKasDao()
                    .getByShift(f.shiftId)
                    .filter { it.jenis == JenisPergerakanKas.PENJUALAN }
                    .sumOf { it.jumlahDelta },
            )
        }
}
