...
/**
 * Dialog pembayaran (spek 4.3): pilih metode via SegmentedButton
...
fun DialogPembayaran(
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                enabled = !sedangProses && (metode == 1 || cukup),
                onClick = {
                    if (metode == 1) onKonfirmasiQris() else onKonfirmasiTunai(diterima)
                },
            ) {
                Text(
                    if (sedangProses) "Memproses…" else "Selesaikan",
                    fontWeight = FontWeight.Bold,
                )
            }
        },
