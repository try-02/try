...
# Gunakan 'apt' (bukan 'pkg') karena di container berjalan sebagai root
RUN apt update && \
    apt install -y ${NODE_PKG} python make clang build-essential tar

WORKDIR /build

# Gunakan cache mount untuk npm agar download dependensi instan
